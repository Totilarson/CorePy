...
sns.scatterplot(x=Run_settings["Elements_plotted"][0], y=Run_settings["Elements_plotted"][0], hue=Run_settings["RockClassification"],data=coredata, palette=chemofacies_color,ax=ax12, edgecolor='black')
ax12.legend([])
...
for i in range(len(coredata)):
    Q = [0, 0, coredata["Chemofacies_NN"][i]+2, coredata["Chemofacies_NN"][i]+2]
    ...
    plt.fill(Q, Z,c=chemofacies_color[coredata["Chemofacies_NN"][i]], linewidth=0.0)
...
plt.subplot(1, n, 2)
for i in range(len(coredata)):
    Q = [0, 0, coredata["Chemofacies_XGB"][i]+2, coredata["Chemofacies_XGB"][i]+2]
    Z = [coredata[Run_settings["Depth_model"]][i]+Corebeta["XRF_resolution"], coredata[Run_settings["Depth_model"]][i], coredata[Run_settings["Depth_model"]][i], coredata[Run_settings["Depth_model"]][i]+Corebeta["XRF_resolution"]]
       
    plt.fill(Q, Z,c=chemofacies_color[coredata["Chemofacies_XGB"][i]], linewidth=0.0)
    plt.ylim((max(coredata[Run_settings["Depth_model"]]),min(coredata[Run_settings["Depth_model"]])))
    plt.xlim((0,6))
    #plt.xlabel("Chemofacies_NN", fontsize=18)
    plt.xlabel('Rockclass_XGB', fontsize=18)
    plt.ylabel(Run_settings["Depth_model"], fontsize=18)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)  
    #plt.ylim(10892,10820)
