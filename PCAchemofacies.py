...
# Calls settings.py variables for directory handling
...
# Sort coredata by depth and select Formation(s) of interest
...
kmeans = KMeans(n_clusters=settings.clusters) #select the number of clusters (From settings.py)
...
X['Chemofacies']=Chemofacies # Appends a new column based on above conditional format

# Scaling used for plotting PCA
xs = x_new[:,settings.PCA1]
ys = x_new[:,settings.PCA2]
scalex = 1.0/(xs.max() - xs.min())
scaley = 1.0/(ys.max() - ys.min())

# Plot PCA biplot with k-means clusters identified
...
# Save Biplot figure to output directory
...
# Plot a visualization of the outliers. For this example,Al vs. Si and Ca vs. Sr are chosen
